...
fn main() {
    ...
    for mov in moves {
        ...
        let mut origin_group: Vec<char> = Vec::new();

        for _ in 0..count {
            origin_group.push(stacks.get_mut(start - 1).unwrap().pop().unwrap());
        }

        origin_group.reverse();

        stacks[end - 1].append(&mut origin_group);
    }
