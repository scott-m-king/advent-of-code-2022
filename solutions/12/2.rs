...
fn main() {
    ...
    let mut start_pos: Vec<Pos> = Vec::new();
    ...
    for (row, item) in items.iter().enumerate() {
        let start = item.iter().position(|c| *c == 'S' || *c == 'a');
        match start {
            Some(col) => start_pos.push((row as i32, col as i32)),
            None => {}
        }
    }

    // we love a brute force moment
    let result = start_pos.iter().map(|pos| bfs(&items, *pos)).min().unwrap();
    println!("{}", result);
}
